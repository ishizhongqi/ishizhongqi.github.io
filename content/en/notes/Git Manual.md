...
*[➤ 原作者仓库入口](https://github.com/xjh22222228/git-manual/)*  
___
...
git clone -b develop --single-branch https://github.com/username/example.git

# 指定克隆后的文件夹名称
git clone https://github.com/username/example.git git-study # 如果后面是 . 在当前目录创建
...
git clone --depth=1 --no-single-branch https://github.com/username/example.git # --no-single-branch 同时克隆其他所有分支


# 裸克隆, 没有工作区内容，不能进行提交修改，一般用于复制仓库
git clone --bare https://github.com/username/example.git

# 镜像克隆, 也是裸克隆, 区别于包含上游版本库注册
git clone --mirror https://github.com/username/example.git
```
...
git remote add example https://github.com/username/example.git
...
都说用 `git rebase` 代替 `git merge` 进行合并，这2个区别在于 `git rebase` 可以使历史记录更清晰。  
...
```bash
git submodule add https://github.com/username/example.git # 默认添加到当前目录下
git submodule add https://github.com/username/example.git submodules/git-manual  # 添加到指定目录

# -b 指定需要添加仓库的某个分支
git submodule add -b develop https://github.com/username/example.git
```
...
```bash
git subtree add --prefix=sub/common https://github.com/username/example.git main --squash
```
...
```bash
git subtree pull --prefix=sub/common https://github.com/username/example.git main --squash
```
...
git commit -m "子仓库修改"
# 然后推送
git subtree push --prefix=sub/common https://github.com/username/example.git main --squash
```
...
```bash
# common 是仓库名字，可以随意定义
git remote add -f common https://github.com/username/example.git
```
...
```bash
alias push="git subtree push --prefix=sub/common https://github.com/username/example.git main --squash"
```
...
{
  "scripts": {
    "push": "git subtree push --prefix=sub/common https://github.com/username/example.git main --squash"
  }
}
```
...
例子：
```bash
git log -n 1 --pretty=format:"%an" # name

git log -n 1 --pretty=format:"%ae" # name@example.com
...
```bash
# 克隆裸仓库，里面没有工作区内容
git clone --bare https://github.com/username/exampleOld.git
```

2、镜像推送至新仓库
```bash
cd A
git push --mirror https://github.com/username/exampleNew.git
```
...
4、拉取新仓库
```bash
git clone https://github.com/username/exampleNew.git
```
